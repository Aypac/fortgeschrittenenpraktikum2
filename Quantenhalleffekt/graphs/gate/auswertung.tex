...
\begin{document}
	...
	] {../data/gate/ns_und_mu_1.csv};
	
	\addplot[color=orange, mark=x, %only marks,
	%error bars/.cd, error mark=-, x dir=both, x explicit, y dir=both, y explicit
	]
	table[x index=0, y expr={\thisrowno{6}*200}, skip first n=1, x error expr=\xerr, y error index=7
	] {../data/gate/ns_und_mu_2.csv};
...
%	] {../data/gate/ns_und_mu.csv};
	...
	\begin{axis}[
		...
		\addlegendentry{$\mu_{\downarrow}$}
		\addplot[color=orange, mark=x, only marks, domain=-300:-299, samples=2] {0}; %dummy
		\addlegendentry{$\mu_{\uparrow} \cdot 200$}
