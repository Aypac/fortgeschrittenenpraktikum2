...
\begin{enumerate}
	...
	\label{eq:umdreh}
	\end{equation}\textit{Beweisskizze:} Vorzeichenumkehr $x \rightarrow -x$ in \eqref{eq:ift} entspricht der FT von $G(s)$ und $F\left\{ G(s) \right\} = F\left\{ F\left\{F^{-1}\{G(s)\}\right\} \right\} = F\left\{ F\left\{g(x)\right\} \right\}$.
